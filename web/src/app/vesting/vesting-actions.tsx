    ...
    <section className="space-y-4">
      ...
      {!writesAllowed && (
        <p className="rounded-md border border-warning/40 bg-warning/10 p-3 text-xs text-warning">
          ...
          rendered against testnet / dev-net deployments. Once a mainnet
          Vesting contract goes live, our on-chain observer catches it in the
          next{' '}
      ...
      {isConnected && vesting?.exists && (
        <div className="card space-y-4 p-6">
          <div className="grid gap-3 sm:grid-cols-4">
            ...
            />
            <Stat label="Vested %" value={`${(vestedFrac * 100).toFixed(1)}%`} />
          </div>
