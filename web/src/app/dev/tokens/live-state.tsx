  ...
  return (
    <>
      ...
      <dd
        className={cn(
          "font-mono",
          valueStr.length > 50 ? "truncate text-xs" : "",
        )}
      >
        {valueStr}
      </dd>
...
function TokenState({
  ...
  if (!address)
    return (
      <p className="text-sm text-muted-foreground">
        {label}: not deployed on {network}.
      </p>
    );
  if (isLoading)
    return (
      <p className="text-sm text-muted-foreground">Loading {label} state…</p>
    );
  if (error)
    return (
  ...
  const totalSupplyField = authorityIsAddress
    ? data.mutFields[1]
    : data.mutFields[0];
  const rawSupply =
    totalSupplyField && totalSupplyField.type === "U256"
...
function OracleState({ network }: { network: Network }) {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const priceReturn = (
        result as { returns: Array<{ type: string; value: string }> }
      ).returns[0];
  ...
  if (!address)
    return (
      <p className="text-sm text-muted-foreground">
        DIA oracle: not deployed on {network}.
      </p>
    );
  if (isLoading)
    return <p className="text-sm text-muted-foreground">Loading oracle…</p>;
  if (error)
    return (
      <p className="text-sm text-destructive">
...
function PlatformSettingsState({ network }: { network: Network }) {
  ...
  if (!address)
    return (
      <p className="text-sm text-muted-foreground">
        PlatformSettings: not deployed on {network}.
      </p>
    );
  if (isLoading)
