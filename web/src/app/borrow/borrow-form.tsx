...
function useAlphPrice(refreshMs = 30_000): PriceState {
  ...
  useEffect(() => {
    if (!adapter) {
      setState({ price: null, lastUpdatedMs: null, isLoading: false, error: null })
      return
    }
    let cancelled = false
    ...
    async function fetchOnce(): Promise<void> {
      try {
        ...
        if (!cancelled)
          setState({ price, lastUpdatedMs: Date.now(), isLoading: false, error: null })
      } catch (err) {
        if (!cancelled)
          setState((prev) => ({
            ...prev,
...
export function BorrowForm() {
  ...
  const { price: priceUsdPerAlph, lastUpdatedMs, isLoading, error } = useAlphPrice()
  ...
  const metrics = useMemo(() => {
    ...
    const crPercent = debtNum > 0 ? (netCollateralUsd / debtNum) * 100 : Infinity
    const liquidationPriceUsdPerAlph =
      netCollateralAlph > 0 ? (debtNum * MCR) / netCollateralAlph : 0

    const maxBorrowAtMcr = debtNum > 0 ? collateralUsd / MCR : collateralUsd / MCR
    const valid =
      collNum > 0 &&
  ...
  const zone = metrics && isFinite(metrics.crPercent) ? crZone(metrics.crPercent) : null
  ...
  async function handleSubmit(e: React.FormEvent): Promise<void> {
    e.preventDefault()
    if (!writesAllowed || !isConnected || !wallet.signer || !metrics?.valid) return
    ...
    <div className="grid gap-8 lg:grid-cols-2">
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        <div className="space-y-2">
          ...
          {metrics && (
            <p className="text-xs text-muted-foreground">
              Worth{' '}
              <span className="font-mono">${formatUsd(metrics.collateralUsd, 2)}</span> at the
        ...
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-muted-foreground">
            Minimum loan size: <span className="font-mono">{MIN_LOAN_ABD}</span> ABD.
          </p>
          {maxHint && (
            <p className="text-xs text-warning">{maxHint}</p>
          )}
        </div>
        ...
        <fieldset className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            Lower rates are redeemed first when ABD holders redeem against the protocol.
            Higher rates reduce your exposure to forced redemption but cost more over time.
          </p>
        </fieldset>
        ...
        <section className="space-y-2">
          ...
          {priceUsdPerAlph != null && (
            <p
              className={cn(
                'font-mono text-2xl',
                isPriceStale ? 'text-muted-foreground line-through' : 'text-primary',
              )}
            >
        ...
        {metrics && isFinite(metrics.crPercent) && zone && (
          <section className="space-y-3">
            ...
            <div className="flex items-baseline gap-3">
              <span className="font-mono text-3xl">{metrics.crPercent.toFixed(2)}%</span>
              <span
                className={cn(
            ...
            <p className="text-xs text-muted-foreground">
              Zones: Conservative ≥ 400 %, Moderate 280&ndash;400 %, Aggressive 230&ndash;280 %,
              High Risk 200&ndash;230 %, Liquidation &lt; 200 %.
            </p>
          </section>
        )}
          ...
          <section className="space-y-2">
            ...
            <dl className="grid grid-cols-[16ch_1fr] gap-x-4 gap-y-1 text-sm">
              ...
              <dd className="font-mono">
                ...
                <span className="text-muted-foreground">(${formatUsd(metrics.feeUsd)})</span>
              </dd>
              ...
              <dd className="font-mono">
                {tier}% ({((Number(borrowAbd) || 0) * (tier / 100)).toFixed(2)} ABD / year){' '}
              </dd>
            </dl>
            {!metrics.valid && (
