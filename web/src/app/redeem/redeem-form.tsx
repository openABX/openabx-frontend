...
export function RedeemForm() {
  ...
  const canSubmit =
    isConnected && writesAllowed && atto > 0n && addrValidation.ok && !isBusy;
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        <div className="space-y-2">
          ...
          {balances?.abdAtto != null && (
            ...
            >
              Use wallet balance: {formatAmount(balances.abdAtto, 9, 2)} ABD
            </button>
          )}
        </div>
        ...
        {!writesAllowed ? (
          <div className="rounded-md border border-warning/40 bg-warning/10 p-3 text-xs text-warning">
            <p className="font-medium">
              Mainnet redemption path under construction.
            </p>
            <p className="mt-1">
      ...
      <aside className="card space-y-4 p-6">
        <section>
          ...
          <dl className="mt-2 grid grid-cols-[14ch_1fr] gap-x-4 gap-y-1 text-sm">
            ...
            <dd className="font-mono">${formatUsd(netAbd)}</dd>
        ...
        <section className="rounded-md border border-warning/40 bg-warning/10 p-3 text-xs text-warning">
          <p>
            Redemptions pull ALPH from the targeted loan and burn the ABD. When
            protocol CR falls below 100%, the contract returns ALPH pro-rata
            instead of 1:1 — so the net you receive may be less than the
            preview.
          </p>
        </section>
      </aside>
