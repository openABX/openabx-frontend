...
export function LiquidateForm() {
  ...
  const atRisk = (loans ?? [])
    .filter(
      (l) => !l.liquidatable && l.crBps != null && l.crBps < 25_000,
    )
    ...
    <div className="space-y-8">
      ...
      <section className="space-y-3">
        ...
        {liquidatable.length === 0 ? (
          <p className="rounded-md border border-dashed border-border bg-[hsl(var(--surface-2))]/50 p-4 text-sm text-muted-foreground">
            No loans currently below the 200% MCR. The scanner walks the 30
            most recent LoanManager-touching addresses; lower-CR loans may
      ...
      {!writesAllowed && (
        <section className="card space-y-3 p-5">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            LoanManager method index for liquidation has not yet been
            observable on-chain (no liquidation tx has occurred on mainnet
            since launch — every active loan is above the 200 % MCR).
            Once the first liquidation lands, we catalogue the template and
            enable the button on this page.
          </p>
          <p className="text-sm text-muted-foreground">
            ...
            — we ship a live watchlist above so you can find at-risk loans
            and monitor when they cross the threshold.
          </p>
        </section>
      )}
      ...
      <section className="space-y-2 rounded-lg border border-dashed border-border bg-[hsl(var(--surface-2))]/50 p-4 text-sm">
        ...
        <ol className="list-inside list-decimal space-y-1 text-muted-foreground">
          <li>
            You call <span className="font-mono">liquidate(owner)</span>{' '}
            against the target loan.
          </li>
          <li>
            LoanManager reads the oracle, verifies CR &lt; 200%, and
            forwards debt + collateral to AuctionManager.
          </li>
          <li>
