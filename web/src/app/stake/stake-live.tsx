      ...
      <section className="grid gap-4 sm:grid-cols-4">
        <Card label="ABX circulating" valueMono>
          {abxTotalSupply != null
            ...
            : '—'}
          <span className="ml-1 text-sm text-muted-foreground">{abxSymbol ?? 'ABX'}</span>
        </Card>
        <Card label="ALPH / USD">
          <span
            className={
        ...
        <Card label="Fee share → stakers">
          <span className="font-mono text-2xl text-primary">
            {stakerSharePct != null ? `${(stakerSharePct * 100).toFixed(1)}%` : '—'}
          </span>
      ...
      <section className="space-y-4">
        ...
        <div className="card p-4 text-sm">
          ...
          <ul className="mt-2 list-inside list-disc space-y-1 text-muted-foreground">
            ...
            <li>
              <span className="font-mono">k</span> goes to ABX stakers,
              pro-rata to staked ABX.
            </li>
          </ul>
...
function Card({ label, children, valueMono }: CardProps) {
  return (
    <div className="card p-5">
      ...
      <div className={valueMono ? 'mt-2 font-mono text-2xl' : 'mt-2 text-2xl'}>{children}</div>
    </div>
  )
}
