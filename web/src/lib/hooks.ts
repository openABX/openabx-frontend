...
function useConnectedAddress(): string | null {
  ...
  return wallet.connectionStatus === 'connected' ? wallet.account.address : null
}
...
export function useLoanPosition(): UseQueryResult<LoanPosition> {
  ...
  return useQuery({
    ...
    queryFn: () =>
      address ? fetchLoanPosition(NETWORK, address) : Promise.resolve(EMPTY_LOAN),
    enabled: userQueryEnabled(address),
    refetchInterval: 30_000,
  })
}
...
export function useStakePosition(): UseQueryResult<StakePosition> {
  ...
  return useQuery({
    ...
    queryFn: () =>
      address ? fetchStakePosition(NETWORK, address) : Promise.resolve(EMPTY_STAKE),
    enabled: userQueryEnabled(address),
    refetchInterval: 30_000,
  })
}
