...
describe('Vesting', () => {
  ...
  it('exposes constructor state via getters', async () => {
    ...
    const alloc = await Vesting.tests.getTotalAllocated({ initialFields: fields })
    expect(alloc.returns).toBe(0n)
    const claimed = await Vesting.tests.getTotalClaimed({ initialFields: fields })
    expect(claimed.returns).toBe(0n)
    const twelve = await Vesting.tests.getTwelveMonthsMs({ initialFields: fields })
