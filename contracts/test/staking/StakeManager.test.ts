...
describe("StakeManager", () => {
  ...
  it("exposes constructor state via getters", async () => {
    ...
    const cooldown = await StakeManager.tests.getUnstakeCooldownMs({
      initialFields: fields,
    });
    expect(cooldown.returns).toBe(FOURTEEN_DAYS_MS);
    const total = await StakeManager.tests.getTotalStakedAbx({
      initialFields: fields,
    });
    expect(total.returns).toBe(0n);
    const idx = await StakeManager.tests.getRewardIndex({
      initialFields: fields,
    });
  ...
  describe("notifyRewards math", () => {
    ...
    it("rewardIndex accumulates across multiple notifications", async () => {
      ...
      expect(state.fields.rewardIndex).toBe(
        startIndex + (reward * PRECISION) / totalStaked,
      );
    });
