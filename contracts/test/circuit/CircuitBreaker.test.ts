...
describe('CircuitBreaker', () => {
  ...
  it('read-only accessors reflect constructor state', async () => {
    ...
    const isPaused = await CircuitBreaker.tests.isPaused({ initialFields: fields })
    expect(isPaused.returns).toBe(false)

    const pauser = await CircuitBreaker.tests.getPauser({ initialFields: fields })
    expect(pauser.returns).toBe(aliceAddress)

    const stale = await CircuitBreaker.tests.getOracleStalenessMillis({ initialFields: fields })
    expect(stale.returns).toBe(1_800_000n)
  })

  it('requireNotPaused succeeds when not paused', async () => {
    await expect(
      CircuitBreaker.tests.requireNotPaused({ initialFields: baseFields({ paused: false }) }),
    ).resolves.toBeDefined()
  })

  it('requireNotPaused reverts with Paused (101) when paused', async () => {
    await expect(
      CircuitBreaker.tests.requireNotPaused({ initialFields: baseFields({ paused: true }) }),
  ...
  it('pauser can pause and unpause', async () => {
    ...
    const s1 = paused.contracts.find(
      (c): c is CircuitBreakerTypes.State => c.codeHash === CircuitBreaker.contract.codeHash,
    )
