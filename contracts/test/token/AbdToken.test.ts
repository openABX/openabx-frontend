...
describe("AbdToken", () => {
  ...
  it("exposes symbol, name, decimals, totalSupply", async () => {
    const symbol = await AbdToken.tests.getSymbol({
    ...
    expect(Buffer.from(name.returns, "hex").toString("utf-8")).toBe(
      "ABD Token",
    );

    const decimals = await AbdToken.tests.getDecimals({
      initialFields: baseFields(),
    });
    ...
    const authority = await AbdToken.tests.getMintAuthority({
      initialFields: baseFields(),
    });
  ...
  it("mint by authority increases totalSupply and transfers tokens out", async () => {
    ...
    const state = result.contracts.find(
      (c): c is AbdTokenTypes.State =>
        c.codeHash === AbdToken.contract.codeHash,
    );
    ...
    const deliveredToBob = result.txOutputs.find(
      (o) =>
        o.address === bobAddress &&
        o.tokens?.some((t) => t.id === fake.tokenId),
    );
    expect(deliveredToBob).toBeDefined();
    const bobAbdBalance = deliveredToBob!.tokens!.find(
      (t) => t.id === fake.tokenId,
    )!.amount;
        ...
        inputAssets: [
          {
            ...
            asset: {
              alphAmount: ONE_ALPH,
              tokens: [{ id: fake.tokenId, amount: 1n }],
            },
          },
        ],
