...
describe('AbxToken', () => {
  ...
  it('exposes symbol, name, decimals, totalSupply', async () => {
    const symbol = await AbxToken.tests.getSymbol({ initialFields: baseFields() })
    ...
    const decimals = await AbxToken.tests.getDecimals({ initialFields: baseFields() })
    expect(decimals.returns).toBe(9n)

    const supply = await AbxToken.tests.getTotalSupply({ initialFields: baseFields() })
  ...
  it('self-burn by the holder decreases totalSupply', async () => {
    ...
    const state = result.contracts.find(
      (c): c is AbxTokenTypes.State => c.codeHash === AbxToken.contract.codeHash,
    )
