...
function expectedAlphForDebt(
  ...
  return (debt * (BPS_SCALE + discountBps) * PRECISION * ABD_SCALE) / (BPS_SCALE * price)
}
...
describe('AuctionManager', () => {
  ...
  it('exposes configured fields via getters', async () => {
    ...
    const admin = await AuctionManager.tests.getAdmin({ initialFields: fields })
    expect(admin.returns).toBe(aliceAddress)
    const lm = await AuctionManager.tests.getLoanManager({ initialFields: fields })
  ...
  it('isWired is false when any reference is empty', async () => {
    const r1 = await AuctionManager.tests.isWired({ initialFields: baseFields() })
    ...
    const r2 = await AuctionManager.tests.isWired({ initialFields: missingOne })
    expect(r2.returns).toBe(false)
  })

  it('isWired is true when all references are set', async () => {
    const r = await AuctionManager.tests.isWired({ initialFields: wiredFields() })
  ...
  describe('computePoolShare', () => {
    ...
    it('pool absorbs full desired debt when it has capacity and ALPH is sufficient', async () => {
      ...
      const expectedAlph = expectedAlphForDebt(desiredDebt, 500n, PRICE_5_CENTS)
    ...
    it('clamps debt to poolTotalAbd when desired exceeds pool size', async () => {
      ...
      const expectedAlph = expectedAlphForDebt(poolTotalAbd, 1000n, PRICE_5_CENTS)
