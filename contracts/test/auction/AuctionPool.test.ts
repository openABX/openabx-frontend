...
describe("AuctionPool", () => {
  ...
  it("exposes configured fields via getters", async () => {
    ...
    const disc = await AuctionPool.tests.getDiscountBps({
      initialFields: fields,
    });
    expect(disc.returns).toBe(1500n);
    const fee = await AuctionPool.tests.getBidSuccessFeeBps({
      initialFields: fields,
    });
    expect(fee.returns).toBe(150n);
    const close = await AuctionPool.tests.getCloseBidFeeBps({
      initialFields: fields,
    });
    ...
    const total = await AuctionPool.tests.getTotalAbd({
      initialFields: fields,
    });
