...
const DEFAULT_DIA_TESTNET_REGISTRY =
...
const deploy: DeployFunction<Network<unknown>> = async (
  deployer: Deployer,
  network: Network<unknown>,
): Promise<void> => {
  const { adminAddress } = getDeployerAddresses(deployer);
  const diaRegistry =
    process.env["DIA_REGISTRY_ID"] ?? DEFAULT_DIA_TESTNET_REGISTRY;
  ...
  const abdPriceOracle = await deployContractAny(deployer, AbdPriceOracle, {
    initialFields: {
      precision: PRECISION,
    } satisfies AbdPriceOracleTypes.Fields,
  });
  ...
  console.log(
    `AuctionManager:      ${auctionManager.contractInstance.address}`,
  );
  ...
  console.log(
    `AuctionFarming:      ${auctionFarming.contractInstance.address}`,
  );
  console.log(
    `PlatformSettings:    ${platformSettings.contractInstance.address}`,
  );
  console.log(
    `CircuitBreaker:      ${circuitBreaker.contractInstance.address}`,
  );
  console.log(`DIA adapter:         ${diaAdapter.contractInstance.address}`);
  console.log(
    `ABD oracle:          ${abdPriceOracle.contractInstance.address}`,
  );
  console.log(`\n=== REQUIRED NEXT STEP ===`);
