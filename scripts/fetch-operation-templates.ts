...
async function main(): Promise<void> {
  ...
  for (const target of TARGETS) {
    ...
    if (!entry) {
      console.warn(`  ${target.operation}: no map entry for ${target.contract}`)
      continue
    }
    ...
    const best =
      forMi.find((e) =>
        e.operation.toLowerCase().includes(target.operation.toLowerCase().slice(0, 5)),
      ) ?? forMi[0]
    if (!best) continue
    ...
    const decoded = {
      ...
      methods: script.methods.map((m, i) => ({
        ...
        instrs: m.instrs.map((ins) => {
          ...
          const v = (ins as { value?: unknown; index?: number; offset?: number; selector?: number }).value
          if ('index' in ins) base['index'] = (ins as { index: number }).index
          if ('offset' in ins) base['offset'] = (ins as { offset: number }).offset
          if ('selector' in ins) base['selector'] = (ins as { selector: number }).selector
          if (v !== undefined) {
            if (v instanceof Uint8Array) {
              ...
              base['value'] = JSON.parse(
                JSON.stringify(v, (_k, val) =>
                  typeof val === 'bigint' ? val.toString() : val instanceof Uint8Array ? binToHex(val) : val,
                ),
              )
            }
          }
