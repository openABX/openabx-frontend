...
async function main(): Promise<void> {
  ...
  if (recent.length === 0) {
    ...
    console.log(
      `  Hint: the explorer-backend /addresses/<abxToken>/transactions`,
    );
    console.log(
      `        endpoint returns txs where ABX is the INPUT ADDRESS, not`,
    );
    console.log(
      `        where the ABX token was TRANSFERRED. Your stake moved`,
    );
    console.log(
      `        ABX from your wallet to StakeManager — it's keyed on your`,
    );
    console.log(`        wallet, not on the token. Re-run with your wallet`);
    console.log(
      `        address:  pnpm tsx scripts/find-recent-stake.ts 30 <wallet>`,
    );
    ...
    if (wallet) {
      ...
      console.log(
        `  wallet total=${walletTxs.length}  within window=${rec2.length}`,
      );
    ...
    for (const [i, m] of script.methods.entries()) {
      ...
      for (const ins of m.instrs) {
        if (ins.name === "BytesConst") {
          ...
          lastBytes = v instanceof Uint8Array ? binToHex(v) : (v as string);
