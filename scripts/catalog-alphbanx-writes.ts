...
async function fetchTxsPage(address: string, page: number): Promise<ExplorerTx[]> {
...
function classify(family: TargetContract['family'], delta: TokenDelta): Classification {
...
async function main(): Promise<void> {
  ...
  for (const t of TARGETS) {
    perContract.set(t.address, { role: t.role, family: t.family, methods: new Map() })
  }
  ...
  for (const src of SCAN_SOURCES) {
    if (Date.now() - startedAt > TOTAL_BUDGET_MS) {
      console.warn(`  wall-time budget exhausted — stopping scan at ${src.slice(0, 10)}…`)
      break
    }
    let txs: ExplorerTx[] = []
    ...
    try {
      ...
      console.warn(`[source ${src.slice(0, 10)}…] fetch error: ${(err as Error).message}`)
      continue
    }
    console.log(
      `[source ${src.slice(0, 10)}…] scanning ${txs.length} txs`,
    )
    ...
    for (const tx of txs) {
      ...
      for (const c of calls) {
        if (!addressByContractId.has(binToHex(contractIdFromAddress(c.address)))) {
          ...
          if (row.userDeltaSignatures.length < 10 && !row.userDeltaSignatures.includes(sig))
