    ...
    for (const m of script.methods) {
      ...
      for (const ins of m.instrs) {
        if (ins.name === 'BytesConst') {
          ...
          const hex =
            v instanceof Uint8Array ? binToHex(v) : typeof v === 'string' ? v : ''
...
function tokenDelta(tx: ExplorerTx, addr: string): { abd: bigint; alph: bigint } {
  let abd = 0n
  let alph = 0n
  for (const inp of tx.inputs ?? []) {
    if (inp.contractInput || inp.address !== addr) continue
    alph -= BigInt(inp.attoAlphAmount ?? '0')
    for (const t of inp.tokens ?? []) if (t.id === ABD_ID_HEX) abd -= BigInt(t.amount)
  }
  for (const o of tx.outputs ?? []) {
    if (o.address !== addr) continue
    alph += BigInt(o.attoAlphAmount ?? '0')
    for (const t of o.tokens ?? []) if (t.id === ABD_ID_HEX) abd += BigInt(t.amount)
  }
  return { abd, alph }
}
...
async function main(): Promise<void> {
  ...
  for (const tx of txsByHash.values()) {
    ...
    const signer = tx.inputs?.find((i) => !i.contractInput && i.address)?.address
    if (!signer) continue
    ...
    else if (alphIn && !abdOut && ownLoan) cls = 'closeLoan / withdrawCollateral'
    else cls = 'unknown'
